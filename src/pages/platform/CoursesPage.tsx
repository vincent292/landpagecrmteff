...
import { formatDateTimeLine, getDisplayCity } from "../../utils/publicContent";
...
export function CoursesPage() {
  ...
  const filteredCourses = useMemo(
    () => courses.filter((course) => city === "Todas" || getDisplayCity(course.city) === city),
    [city, courses]
  );
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-16 md:px-8 md:py-24">
      ...
      <div className="mt-12">
        ...
        {!loading && !error && filteredCourses.length === 0 ? <EmptyState /> : null}
          ...
          {filteredCourses.map((course) => (
            <article key={course.id} className="overflow-hidden rounded-[30px] border border-[var(--color-border)] bg-white/60 shadow-[0_18px_48px_rgba(110,74,47,0.08)]">
              ...
              <div className="p-6">
                ...
                <p className="mt-3 text-sm leading-7 text-[var(--color-copy)]">{course.short_description ?? course.description}</p>
                <div className="mt-5 grid gap-2 text-sm text-[var(--color-copy)] sm:grid-cols-2">
                  <span>{formatDateTimeLine(course.start_date, course.start_time)}</span>
