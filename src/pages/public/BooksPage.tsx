...
export function BooksPage() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-16 md:px-8 md:py-24">
      ...
      <h1 className="font-display mt-3 text-5xl font-semibold text-[var(--color-ink)] md:text-6xl">
      ...
      <p className="mt-5 max-w-3xl text-sm leading-7 text-[var(--color-copy)] md:text-base">
        Aquí se publican los libros activos del consultorio con su detalle, precio y acceso controlado mediante compra verificada y token de descarga.
      </p>

      <div className="mt-8 rounded-[30px] border border-[var(--color-border)] bg-white/75 p-5 shadow-[0_18px_50px_rgba(62,42,31,0.08)]">
        <p className="text-sm font-semibold text-[var(--color-ink)]">¿Ya compraste uno de los libros?</p>
        <p className="mt-2 text-sm leading-7 text-[var(--color-copy)]">
          Ingresa tu token para validar la descarga sin exponer el archivo en una URL pública.
        </p>
      ...
      <div className="mt-10">
        ...
        {!loading && !error && items.length === 0 ? <EmptyState label="Todavía no hay libros activos." /> : null}
