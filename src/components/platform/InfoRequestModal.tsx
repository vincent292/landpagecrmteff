...
import { CheckCircle2, MessageCircleMore, X } from "lucide-react";
...
export function InfoRequestModal({
  ...
  useEffect(() => {
    ...
    void getSiteSettings()
      .then((settings) => {
        if (active) setWhatsappNumber(settings.whatsapp || settings.phone);
      })
      .catch(() => {
        if (active) setWhatsappNumber(null);
      });
  ...
  const onSubmit = (values: FormValues) => {
    setSubmitError("");
    const fallbackMessage = values.message?.trim() || buildSuggestedMessage(values.full_name, values.interest_type, values.interest_title);
    const message = renderWhatsAppTemplate(whatsappTemplate, {
    ...
    if (!href) {
      setSubmitError("No encontramos el numero de WhatsApp configurado. Revisa Configuracion del sitio.");
      return;
    }
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="font-display mt-3 text-4xl font-semibold text-[var(--color-ink)]">
              Te llevamos al chat con el mensaje listo.
            </h2>
          </div>
          <button
            type="button"
        ...
        {sent ? (
          <div className="mt-8 rounded-[28px] border border-[rgba(111,122,96,0.22)] bg-[rgba(111,122,96,0.08)] p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-[var(--color-copy)]">
              Envia el mensaje desde WhatsApp para que el equipo pueda responderte desde el CRM.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <Field label="Mensaje para WhatsApp" error={errors.message?.message}>
                <textarea
                  {...messageRegister}
                  onChange={(event) => {
            ...
            <label className="flex gap-3 text-sm leading-6 text-[var(--color-copy)] md:col-span-2">
              ...
              Acepto la política de privacidad y autorizo el contacto por WhatsApp.
            </label>
            ...
            >
              <MessageCircleMore className="inline h-4 w-4" /> {isSubmitting ? "Abriendo..." : "Abrir WhatsApp"}
            </button>
          </form>
        )}
      </div>
