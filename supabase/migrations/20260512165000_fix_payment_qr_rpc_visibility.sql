...
drop function if exists public.set_payment_qr_password(text);
...
drop function if exists public.update_general_payment_qr(text, text);
drop function if exists public.update_general_payment_qr(text, text, text);
...
begin
  ...
  if security_row.id is null then
    raise exception 'Primero ingresa por primera vez la clave del QR.';
  end if;
