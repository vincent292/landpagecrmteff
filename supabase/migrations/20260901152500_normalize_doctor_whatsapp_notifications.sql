-- Normalize local Bolivian doctor phone numbers before enqueueing WhatsApp notifications.
...
begin
  ...
  if doctor_wa_id is not null then
    doctor_body := format(
      'Nueva cita confirmada %s: %s - %s - %s, de %s a %s en %s. Pago verificado. Calendario Apple/Google: %s. Si no puedes atenderla, solicita reprogramacion aqui: %s',
