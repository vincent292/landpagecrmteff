-- Fuera de la ventana de 24 h Meta exige una plantilla. Si la plantilla no
-- tiene encabezado Documento, se envía el enlace seguro que descarga el .ics.
alter table public.crm_settings
  add column if not exists doctor_booking_template_document_header boolean not null default false;

alter table public.crm_notification_outbox
  add column if not exists attachment_template_header boolean not null default false;
...
begin
  ...
  if doctor_wa_id is not null then
    doctor_body := format(
      'Nueva cita confirmada %s: %s · %s · %s, de %s a %s en %s. Pago verificado. Calendario Apple/Google: %s. Si no puedes atenderla, solicita reprogramación aquí: %s',
