alter table public.treatments
  add column if not exists assessment_mode text not null default 'presencial',
  add column if not exists assessment_price_presencial numeric(12,2),
  add column if not exists assessment_price_virtual numeric(12,2);
...
alter table public.treatments
  drop constraint if exists treatments_assessment_mode_check;

alter table public.treatments
  add constraint treatments_assessment_mode_check
  check (assessment_mode in ('presencial', 'virtual', 'ambas'));

alter table public.promotions
  drop constraint if exists promotions_assessment_mode_check;

alter table public.promotions
  add constraint promotions_assessment_mode_check
  check (assessment_mode in ('presencial', 'virtual', 'ambas'));

alter table public.doctor_availability_rules
  drop constraint if exists doctor_availability_rules_care_mode_check;

alter table public.doctor_availability_rules
  add constraint doctor_availability_rules_care_mode_check
  check (care_mode in ('presencial', 'virtual', 'ambas'));

alter table public.appointment_reservations
  drop constraint if exists appointment_reservations_care_mode_check;

alter table public.appointment_reservations
  add constraint appointment_reservations_care_mode_check
  check (care_mode in ('presencial', 'virtual', 'ambas'));

update public.treatments
set
  assessment_mode = coalesce(nullif(trim(assessment_mode), ''), 'presencial'),
  assessment_price_presencial = coalesce(assessment_price_presencial, assessment_price),
  assessment_price_virtual = coalesce(assessment_price_virtual, assessment_price)
where coalesce(requires_assessment, false) = true;
...
set care_mode = coalesce(r.care_mode, ar.care_mode, 'presencial')
from public.doctor_availability_rules r
where ar.availability_rule_id = r.id;

create index if not exists doctor_availability_rules_care_mode_lookup_idx
on public.doctor_availability_rules (city, appointment_type, care_mode, is_active, availability_type, day_of_week, specific_date);

create index if not exists appointment_reservations_rule_slot_idx
on public.appointment_reservations (availability_rule_id, appointment_date, start_time, end_time, status);
